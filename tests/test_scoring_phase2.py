...
def test_score_play_pair_two_aces_hand_mult():
    ...
    snap = _snap({int(HandType.PAIR): 1})
    rng = np.random.default_rng(0)
    # hand: chips +10, mult +2 → 3; pair scores both aces: 11+30 + 11+4 chips/mult parts
    assert score_play(played, snap, rng) == int((10 + 11 + 30 + 11) * (2.0 + 4.0))
...
def test_editions_foil_holo_poly_order():
    ...
    snap = _snap({int(HandType.HIGH_CARD): 1})
    rng = np.random.default_rng(0)
    # hand L1 +5 chips; rank 11 + bonus 30 + foil 50 = 96, mult 1
    assert score_play(played, snap, rng) == 96

    # enhancement Wild (no chip/mult); Holo +10 mult on rank chips only
    played2 = [Card(0, CardEnhancement.WILD, int(Edition.HOLOGRAPHIC))]
    assert score_play(played2, snap, rng) == int((5.0 + 11.0) * 11.0)

    played3 = [Card(0, CardEnhancement.WILD, int(Edition.POLYCHROME))]
    assert score_play(played3, snap, rng) == int((5 + 11) * 1.5)
...
def test_lucky_mult_triggers_with_low_rng():
    ...
    snap = _snap({int(HandType.HIGH_CARD): 1})
    # hand +5 chips; rank 11, mult 1 + 20 = 21
    assert score_play(played, snap, _Rng()) == int((5 + 11) * 21.0)
...
def test_lucky_mult_not_triggered_with_high_rng():
    ...
    played = [Card(0, CardEnhancement.LUCKY, 0)]
    snap = _snap({int(HandType.HIGH_CARD): 1})
    assert score_play(played, snap, _Rng()) == 5 + 11


def test_hand_levels_invalid_negative_level_raises():
    played = [Card(0, 0, 0)]
    snap = GameSnapshot(
        target_score=0,
        current_score=0,
...
def test_hand_levels_zero_level_raises():
    played = [Card(0, 0, 0)]
    snap = _snap({int(HandType.HIGH_CARD): 0})
    rng = np.random.default_rng(0)
    with pytest.raises(ValueError, match=">= 1"):
        score_play(played, snap, rng)
